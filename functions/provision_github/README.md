...
Result:
✅ alice is now org member
❌ alice has ZERO repo access
```
...
**Result:** alice added to `example-corp` org as member (no repo access)
...
**Result:**
- alice added to `developers` team
  - Inherits access to repos: `example-corp/api`, `example-corp/shared-lib`
- alice added to `frontend-team`
  - Inherits access to repos: `example-corp/web-app`, `example-corp/mobile-app`
...
│
└── Members:
    ├── alicejones (member of: developers, frontend-team)
    ├── bjones (member of: developers)
...
DynamoDB provisioning_state table:
{
  ...
  "repos_with_access": [  # Calculated from team permissions
    "example-corp/api",
    "example-corp/web-app"
  ]
}

Compliance question: "What repos can alice access?"
